...
Cross-reference when stakeholder mapping surfaces downstream needs:

- **hs-product-marketing-context** → apex skill; source of ICP, GTM motion, positioning
- **hs-gaccs-brief** → map complete → build the comms brief from it here
- **hs-ci-stakeholder-briefing** → exec briefing output → route here for competitive framing
- **hs-pre-mortem** → blockers surface → pre-mortem the stakeholder failure mode here
- **hs-gtm-launch-planner** → launch tier confirmed → stakeholder map syncs with launch weight
- **hs-brainstorm-okrs** → accountability gaps surface → ownership clarity → route here for OKR alignment
...
Before every session, read `knowledge/INDEX.md`.
Load confirmed rules into intake before asking questions.
Apply rules silently — do not narrate them to the user.

At session end, extract new patterns and store:

```
knowledge/
  gtm-launch/     → patterns from product launch stakeholder maps
  ci/             → patterns from competitive intelligence contexts
  general/        → cross-initiative patterns
  INDEX.md        → progressive loading router + confirmed rules
```

**Hypothesis → Rule promotion:** confirmed 3+ times across sessions → promote.
**Rule → Hypothesis demotion:** contradicted by new data → demote. Log the contradiction.

Watch For observations from Sprint Cards are the primary source of pattern data.
When a Watch For fires — and the PMM confirms what happened — that is one confirmation.
...
### v2.2.0 — 2026-04-17
Commands block added to SKILL.md: /map, /sprint, /deep-dive, /update, /check.
sample_prompts.md added to package with 8 real-scenario prompt templates.
