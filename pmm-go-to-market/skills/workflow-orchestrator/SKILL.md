...
/workflow-orchestrator "Run full launch workflow for analytics dashboard"
DRI: me
Timeline: 8 weeks
Target: mid-market product teams
Success metric: 30% adoption in 90 days

**Quarterly cycle:**

/workflow-orchestrator "Run Q3 quarterly PMM cycle"
Include: Retro on Q2 launches, position refresh, competitive update, new OKRs

**Market entry:**

/workflow-orchestrator "Run market entry workflow for healthcare vertical"
New segment: Healthcare
Timeline: 12 weeks
ICP: Health systems (100-500 bed)

**Competitive response:**

/workflow-orchestrator "Competitive response to Okta SSO launch"
Competitor: Okta
Product launched: SSO integration
Urgency: High (T2/T3)
...
1. You name the workflow type (or describe the initiative)
2. Orchestrator clarifies:
   - DRI (directly responsible individual)
   - Timeline + deadline
   - Success criteria
   - Key stakeholders
3. Produces "Program Charter" document

### Phase 1: Brain Context Check
1. Reads all brain sections
2. Flags staleness:
   - "Positioning is 8 months old" → will refresh
   - "No competitive intel for Okta" → will build
   - "ICP needs update for new segment" → will rebuild
3. Lists gaps and remediation plan

### Phase 2: Skill Sequencing
1. Determines skill chain for this workflow
2. Sets input/output contracts between skills
3. Produces skill sequence diagram
4. Alerts if missing required context

### Phase 3: Skill Execution
1. Invokes first skill with Program State doc
2. Each skill produces artifact + updates Program State
3. Next skill reads updated Program State
4. Continues until workflow complete

### Phase 4: Cross-Skill Coherence Check
1. Verifies positioning ↔ personas consistency
2. Verifies personas ↔ proof points consistency
3. Verifies proof points ↔ competitive intel consistency
4. Flags contradictions and suggests fixes

### Phase 5: Brain Update
1. Writes all artifacts back to brain
...
3. Timestamps each update

### Phase 6: Master Program Document
1. Compiles all skill outputs into one master doc
2. Includes:
   - Program charter
   - All artifacts (positioning, battlecards, strategy, briefs, etc.)
   - Success metrics
   - Timeline + milestones
   - Open questions
   - Next steps
...
1. After workflow completes, logs to brain Section 7
2. Captures: workflow type, duration, outcomes, issues
3. Future workflows reference this log for learning

---

## What Makes This Different

✅ **State passing:** Each skill gets full context from prior skills
✅ **Brain integration:** Reads context once, writes back at end
✅ **Coherence checking:** Ensures positioning, personas, proof points align
✅ **Master document:** One single source of truth for entire GTM program
✅ **Self-learning:** Future workflows get smarter from past learnings
✅ **Error handling:** Flags missing context before invoking skills
✅ **Flexibility:** Works for any workflow (launch, refresh, new market, etc.)
...
→ Orchestrator will automatically refresh it as part of workflow
...
*Orchestrates all PMM skills into complete GTM workflows.*
*Reads from brain, writes back to brain after completion.*
*Produces master program documents for GTM success.*
